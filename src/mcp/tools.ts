...
export class McpTools {
    ...
    const tools: McpTool[] = [
      {
        ...
        parameters: {
          ...
          properties: {
            instrument_token: {
              type: 'string',
              description: 'Instrument identifier (e.g., NSE_EQ|INFY)'
            },
            transaction_type: {
              type: 'string',
              enum: ['BUY', 'SELL'],
            ...
            product: {
              type: 'string',
              enum: ['D', 'I', 'M'],
              description: 'Product type: D (Delivery), I (Intraday), M (Margin)'
            },
            validity: {
              type: 'string',
              enum: ['DAY', 'IOC'],
  ...
  async executeTool(params: { 
    ...
    try {
      switch (tool_id) {
        case 'place-order':
          return this.placeOrder(toolParams);
        
        case 'cancel-order':
          return this.cancelOrder(toolParams);
        
        case 'get-order':
          return this.getOrder(toolParams);
  ...
  private async placeOrder(params: any): Promise<McpToolResult> {
    ...
    if (!Object.values(UpstoxTransactionType).includes(params.transaction_type)) {
      throw new Error(`Invalid transaction type: ${params.transaction_type}`);
    }
    
    // Validate order type
    if (!Object.values(UpstoxOrderType).includes(params.order_type)) {
      throw new Error(`Invalid order type: ${params.order_type}`);
    }
    
    // Additional validations for specific order types
    if ((params.order_type === UpstoxOrderType.LIMIT || params.order_type === UpstoxOrderType.SL) && typeof params.price !== 'number') {
      throw new Error('Price is required for LIMIT and SL orders');
    }
    
    if ((params.order_type === UpstoxOrderType.SL || params.order_type === UpstoxOrderType.SLM) && typeof params.trigger_price !== 'number') {
